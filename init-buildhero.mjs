...
program
    .version('1.0.0')
    .argument('<project-name>', 'Name of the new BuildHero project')
    .option('-t, --template <template-path>', 'Path to the BuildHero template directory', path.join(__dirname, 'buildhero')) // Default to "buildhero" in the current directory
    .action((projectName, options) => {
        ...
        const spinner = ora(`Initializing BuildHero project '${projectName}'...`).start();
        ...
        try {
            ...
            exec(`npm install`, { cwd: projectPath }, (error, stdout, stderr) => {
                ...
                console.log(stdout);
                spinner.succeed(`BuildHero project '${projectName}' has been initialized successfully!`);
            });
        } catch (err) {
